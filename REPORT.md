...
- Plain text by default. When the user asks for text, deliver plain text without bold, italics, underline, headings, bullet points or any other markup, unless the user explicitly asks for a list or a heading. Never use ` characters either (even if its a file or variable name, just put it in plain text and do not use that character).
