...
- Autonomous forward movement defaults to positive cruise throttle `+0.65` from the live web slider; `VELOCIDAD-MAX-30` applies throttle `0.50` for `3.0 s`, `VELOCIDAD-MAX-90` applies throttle `0.70` for `3.0 s`, and then both return to the slider cruise throttle. Reverse throttle is not emitted by the autonomous controller.
...
- Autonomous sign selection accepts smaller/farther signs by default (`TP2_AUTONOMOUS_MIN_AREA_RATIO=0.003`, `TP2_AUTONOMOUS_NEAR_AREA_RATIO=0.030`); `STOP`, `PROHIBIDO`, `CONOS`, and `VALLA` hold neutral for `TP2_AUTONOMOUS_STOP_HOLD_SEC` (`5.0 s` default), and the controller then ignores those safety signs for `TP2_AUTONOMOUS_STOP_IGNORE_SEC` (`5.0 s` default) so the car can continue instead of stopping repeatedly on the same sign.
...
- Script receives LiDAR payloads (`L`) or LiDAR nested in telemetry `D` when the sensor is connected.
