...
def read_accel():
  ...
  acc_x = (acc_x_raw * 9.8) / (0x8000 / 2.0)
  acc_y = (acc_y_raw * 9.8) / (0x8000 / 2.0)
  acc_z = (acc_z_raw * 9.8) / (0x8000 / 2.0)
...
def show_accel_gyro():
  ...
  try:
    while True:
      ...
      print "accel x_raw = %d, y_raw = %d z_raw = %d" % (-acc_x_raw, acc_y_raw, -acc_z_raw)
      """
      print "accel x = %f, y = %f z = %f" % (-acc_x, acc_y, -acc_z)
      print(-acc_x-acc_z+acc_y)
