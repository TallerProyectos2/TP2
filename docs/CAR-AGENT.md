...
- Web autonomous mode:
  ...
  - autonomous forward movement uses positive cruise throttle `+0.65`; `VELOCIDAD-MAX-30` uses `0.50` and `VELOCIDAD-MAX-90` uses `0.70`; when fresh BMI160 telemetry is available, EPC maps the web cruise throttle to a target speed and applies a bounded PID correction to maintain speed; stop, ambiguity, stale frame or stale inference still force neutral `0.0`; the EPC web UI can change cruise throttle and IMU PID values live
