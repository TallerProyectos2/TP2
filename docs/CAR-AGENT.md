...
- Web autonomous mode:
  ...
  - autonomous forward movement uses positive cruise throttle `+0.65` from the live slider; `VELOCIDAD-MAX-30` applies `0.50` for `3.0 s` and `VELOCIDAD-MAX-90` applies `0.70` for `3.0 s`, then both return to the slider cruise throttle; when fresh BMI160 telemetry is available, EPC maps the web cruise throttle to a target speed and applies a bounded PID correction to maintain speed; stop, ambiguity, stale frame or stale inference still force neutral `0.0`; the EPC web UI can change cruise throttle and IMU PID values live
  ...
  - default sign thresholds are tuned to act on slightly smaller/farther signs; `STOP`, `PROHIBIDO`, `CONOS`, and `VALLA` hold neutral for 5.0 s by default, and the controller then ignores those safety signs for 5.0 s by default so it does not stop twice at the same sign
