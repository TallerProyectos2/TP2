...
- Car web/control runtime:
  ...
  - live web tuning can adjust manual throttle, autonomous cruise/turn pulse values, steering trim, lane-assist correction, IMU speed PID values, LiDAR safety thresholds/sector, and turn compensation at runtime through `/settings`; `POST /settings/defaults` persists the current values to a host-local `TP2_CONTROL_DEFAULTS_PATH` file outside the repository
  - autonomous forward commands are clamped to non-negative throttle and default to the live cruise throttle (`+0.65` by default); `VELOCIDAD-MAX-30` applies `0.50` for `3.0 s`, `VELOCIDAD-MAX-90` applies `0.70` for `3.0 s`, and then both return to the live cruise throttle from `POST /cruise-speed`; with fresh BMI160 telemetry, that throttle is mapped to a target speed and corrected by a bounded PID before LiDAR safety caps are applied
  ...
  - IMU speed assist defaults to enabled (`TP2_IMU_SPEED_CONTROL_ENABLED=1`) and exposes `imu.status`, `imu.estimated_speed_mps`, and `imu.speed_control` in `/status.json`; it adjusts only autonomous forward throttle and does not override camera, lane, STOP, or LiDAR safety fallbacks
  - autonomous inference submits frames every `0.07 s` by default when frames are available
  - autonomous sign filtering accepts smaller/farther signs by default (`TP2_AUTONOMOUS_MIN_AREA_RATIO=0.003`, `TP2_AUTONOMOUS_NEAR_AREA_RATIO=0.030`); `STOP`, `PROHIBIDO`, `CONOS`, and `VALLA` hold neutral for `TP2_AUTONOMOUS_STOP_HOLD_SEC=5.0` by default, then ignore safety signs for `TP2_AUTONOMOUS_STOP_IGNORE_SEC=5.0` by default so the same sign is not handled twice
  - turn signs trigger a full-lock timed 90-degree open-loop maneuver from the first valid confirmed detection, including far detections
- Inference files:
...
- `8088/TCP`: live `coche.py` operator web view with camera, browser control, and inference status
  ...
  - `POST /cruise-speed`: update live autonomous cruise throttle
