...
- Car web/control runtime:
  ...
  - autonomous forward commands are clamped to non-negative throttle and default to `+0.65`; the web UI can update the live cruise throttle with `POST /cruise-speed`; with fresh BMI160 telemetry, that throttle is mapped to a target speed and corrected by a bounded PID before LiDAR safety caps are applied
