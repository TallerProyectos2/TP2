...
- Car web/control runtime:
  ...
  - live web tuning can adjust manual throttle, autonomous cruise/turn pulse values, steering trim, lane-assist correction, LiDAR safety thresholds/sector, and turn compensation at runtime through `/settings`; `POST /settings/defaults` persists the current values to a host-local `TP2_CONTROL_DEFAULTS_PATH` file outside the repository
